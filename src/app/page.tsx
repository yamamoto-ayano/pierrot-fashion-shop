...
export default function Page() {
  ...
  const [categoryKeySel, setCategoryKeySel] = useState<string | null>(null)
  ...
  const filtered = useMemo(() => {
    ...
    if (categoryKey && categoryValue !== ALL) {
      arr = arr.filter((r) => (r[categoryKey] ?? "").toString() === categoryValue)
    }
    ...
    return arr
  }, [rows, q, categoryKey, categoryValue, sortKey, sortDir])
  ...
  return (
    <main className="min-h-dvh p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {headers.length > 0 && (
          <Select
            value={categoryKeySel ?? undefined}
            defaultValue={categoryKey || undefined}
            onValueChange={(v) => { setCategoryKeySel(v); setCategoryValue(ALL); setPage(1) }}
          >
            <SelectTrigger className="w-[220px]">
              <SelectValue placeholder="カテゴリに使う列" />
            </SelectTrigger>
            <SelectContent>
              {headers.map((h) => (
        ...
        {categoryKey && (
          <Select
            value={categoryValue}
            onValueChange={(v) => { setCategoryValue(v); setPage(1) }}
          >
            <SelectTrigger className="w-[220px]">
